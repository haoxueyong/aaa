...
selfApp.controller('selfCtrl', function($scope, $http,$filter,$sce,$httpParamSerializerJQLike) {
    $scope.resetPassword = function(){
        var loading = zeroModal.loading(4);
        $http.get("/user/get-self-password-reset-token").then(function success(rsp){
            ...
            if(rsp.data.status == 'success')
            {
                ...
                zeroModal.show({
                    ...
                    okFn: function() {
                        ...
                        var formData = {
                            method: 'POST',
                            url: '/user/reset-self-password?token='+rsp.data.token,post_data,
                            data:post_data
                        }
                        ...
                        $http({
                            ...
                            if(rsp.data.status == 'success'){
                                ...
                                zeroModal.error({content:'密码重置失败！',contentDetail:'您输入的旧密码不正确。'});
                            }else{
