...
abstract class BaseMigrateController extends Controller
{
     ...
     */
    public function actionUp($limit = 0,$automatic = false)
    {
        ...
        $applied = 0;
        if ($automatic === 'y' || $this->confirm('Apply the above ' . ($n === 1 ? 'migration' : 'migrations') . '?')) {
            foreach ($migrations as $migration) {
     ...
     * @return int the status of the action execution. 0 means normal, other values mean abnormal.
     */
    public function actionRedo($limit = 1,$automatic = false)
    {
        if ($limit === 'all') {
            $limit = null;
        } else {
        ...
        if ($automatic === 'y' || $this->confirm('Redo the above ' . ($n === 1 ? 'migration' : 'migrations') . '?')) {
            foreach ($migrations as $migration) {
                if (!$this->migrateDown($migration)) {
                    $this->stdout("\nMigration failed. The rest of the migrations are canceled.\n", Console::FG_RED);
